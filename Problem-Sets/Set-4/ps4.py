...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
#    pylab.plot(x, y, '.', label = 'training data')
    bestModel = {"model":None, "r2":None, "polyval":None}
    for model in models:
        estimated = np.polyval(model, x)
        r2 = r_squared(y, estimated)
#        pylab.plot(x, estimated, '--', label = 'fit: r2 = ' + "%.2f" % r2)
        if bestModel["model"] is None or bestModel["r2"] < r2:
            bestModel["model"] = model
            bestModel["r2"] = r2
            bestModel["polyval"] = estimated
    
    pylab.plot(x, y, 'b.', label = 'training data')
    pylab.plot(x, bestModel["polyval"], 'r-', label = 'best fit')
    pylab.legend()
    pylab.xlabel("Year")
    pylab.ylabel("Temperature [°C]")
    pylab.title("Best polynomial fit of training data" + "\n" + "Parameters: " + 
                str(bestModel["model"]) + "\n" + "R2: " + "%.4f" % bestModel["r2"])
    pylab.show()
...
y = []
for year in x:
    daily_temps = raw_data.get_yearly_temp("BOSTON", year)
